...
//	        sw_draw_window(-0.35, -0.2, 0.7, 0.4, 0.02);
		...
		{
			if(input->mode == BAM_DRAW)
				sui_draw_text(sui_text_line_length(0.02, 2, "CONNECTING", -1) * -0.5, -0.35, 0.02, 2, "CONNECTING", 0, 0, 0, 1);
		}
		else
		{
			...
			if(input->mode == BAM_DRAW)
				sui_draw_text(sui_text_line_length(0.02, 2, "CONNECT", -1) * -0.5, 0.35, 0.02, 2, "CONNECT", 0, 0, 0, 1);
...
extern void		*lo_preview_geometry_update(ENode *node, void *g);
extern boolean	lo_preview_geometry_draw(void *g, float brightness);
extern void		lo_preview_geometry_destroy(void *g);

void lo_geo_preview_func(ENode *node, ECustomDataCommand command)
{
	if(command == E_CDC_DESTROY)
		lo_preview_geometry_destroy(e_ns_get_custom_data(node, 0));
}

boolean la_pu_empty(BInputState *input)
{
	SUIPUElement element[4];
	static float x, y, static_start = 0, rotate = 0;
	float brightness = 0.5, tmp[2], aspect;
	uint start = 0, ring, count = 4, i;
	boolean preview;
	ENode *node;

	preview = sui_get_setting_int("GEOMETRY_PREVIEW", TRUE);
...
*/		break;
	...
	if(input->mode == BAM_DRAW)
	{
		...
		i = 1;
		glPushMatrix();
		tmp[0] = -(input->pointer_x + 0.8 - 1.0 / 16.0) / (1.6 / (16.0 * 1.6));
		tmp[1] =  -(-input->pointer_y + y - 0.06) / (1.6 / (16.0 * 1.6));
		brightness = 0.5;
		if(tmp[0] * tmp[0] + tmp[1] * tmp[1] < 0.8 * 0.8)
			brightness = 1.0;
		sui_draw_text(sui_text_line_length(SUI_T_SIZE / 2.0 * (16.0 * 1.6), SUI_T_SPACE, "NEW", -1) * -0.5, 0, SUI_T_SIZE / 2.0 * (16.0 * 1.6), SUI_T_SPACE, "NEW", brightness, brightness, brightness, brightness);
		glPopMatrix();
		glEnable(GL_BLEND);
		...
		for(node = e_ns_get_node_next(0, 0, V_NT_GEOMETRY); node != NULL; node = e_ns_get_node_next(e_ns_get_node_id(node) + 1, 0, V_NT_GEOMETRY))
		{
			...
			if((float)(i / 16) * 1.6 * (1.6 / (16.0 * 1.6)) - y > -aspect - 0.2)
			{
				...
				tmp[0] = (float)(i % 16) * 1.6 - (input->pointer_x + 0.8 - 1.0 / 16.0) / (1.6 / (16.0 * 1.6));
				tmp[1] = (float)(i / 16) * 1.6 - (-input->pointer_y + y - 0.06) / (1.6 / (16.0 * 1.6));
				brightness = 0.1;
				if(preview)
				{
					...
					{
						brightness = 0.4;
						sui_draw_text(sui_text_line_length(SUI_T_SIZE / 2.0 * (16.0 * 1.6), SUI_T_SPACE, e_ns_get_node_name(node), -1) * -0.5, - 0.8, SUI_T_SIZE / 2.0 * (16.0 * 1.6), SUI_T_SPACE, e_ns_get_node_name(node), 1.0, 1.0, 1.0, 1.0);
					}
					glRotatef(rotate, 0, 1, 0);
					if(!lo_preview_geometry_draw(e_ns_get_custom_data(node, 0), brightness))
						sui_draw_text(sui_text_line_length(SUI_T_SIZE / 2.0 * (16.0 * 1.6), SUI_T_SPACE, "EMPTY", -1) * -0.5, 0, SUI_T_SIZE / 2.0 * (16.0 * 1.6), SUI_T_SPACE, "EMPTY", brightness, brightness, brightness, brightness);
				}else
				{
					if(tmp[0] * tmp[0] + tmp[1] * tmp[1] < 0.8 * 0.8)
						brightness = 0.4;
					sui_draw_text(sui_text_line_length(SUI_T_SIZE / 3.0 * (16.0 * 1.6), SUI_T_SPACE, e_ns_get_node_name(node), -1) * -0.5, 0, SUI_T_SIZE / 3.0 * (16.0 * 1.6), SUI_T_SPACE, e_ns_get_node_name(node), brightness, brightness, brightness, brightness);
				}
		...
		if(input->pointer_y > aspect * 0.9 && y > aspect * 0.8)
			y -= betray_get_delta_time() * 2;
		if(input->pointer_y < aspect * -0.9 && aspect * 0.8 / (1.6 / (16.0 * 1.6)) < (float)(i / 16) * 1.6 - (y - 0.06) / (1.6 / (16.0 * 1.6)))
			y += betray_get_delta_time() * 2;
		glPopMatrix();
		...
		glPushMatrix();
		glEnable(GL_DEPTH_TEST);
	}else if(input->mouse_button[2] == FALSE && input->last_mouse_button[2] == TRUE)
	{
		tmp[0] = -(input->pointer_x + 0.8 - 1.0 / 16.0) / (1.6 / (16.0 * 1.6));
		tmp[1] = -(-input->pointer_y + y - 0.06) / (1.6 / (16.0 * 1.6));
		if(tmp[0] * tmp[0] + tmp[1] * tmp[1] < 0.8 * 0.8)

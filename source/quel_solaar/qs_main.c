...
//	qs_intro_draw(NULL);
	...
	if(active)
	{
		...
		{
			if(input->mode == BAM_DRAW)
				sui_draw_text(sui_text_line_length(0.02, 2, "CONNECTING...", -1) * -0.5, -0.3, 0.02, 2, "CONNECTING...", 0, 0, 0, 1.0);    
		}
		else
		{
			...
			if(input->mode == BAM_DRAW)
				sui_draw_text(sui_text_line_length(0.02, 2, "CONNECT", -1) * -0.5, 0.35, 0.02, 2, "CONNECT", 0, 0, 0, 1.0);
...
//	betray_set_action_func(sui_symbol_editor_func, NULL);
	betray_set_context_update_func(qs_context_update);

...
uint co_vng_render_name(BInputState *input, const char *name, float pos_x, float pos_y, float pointer_x, float pointer_y)
{
	...
	if(input->mode == BAM_DRAW)
	{
		...
		sui_draw_text(0.6, 0.6 + 0.5 * SUI_T_SIZE * 4, SUI_T_SIZE * 4, SUI_T_SPACE * 2, name, co_line_color[0], co_line_color[1], co_line_color[2], color);
		COVNGraphicsData.name[6] = 0.6 + 0.5 * SUI_T_SIZE * 4 + sui_text_line_length(SUI_T_SIZE * 4, SUI_T_SPACE * 2, name, -1);
...
void co_vng_divider(BInputState *input, float pos_x, float pos_y, float *rotate, float *color, float *soft_color, boolean *on, char *text)
{
	if(input->mode == BAM_DRAW)
	{
		...
		line[4] = 0.05 + sui_text_line_length(SUI_T_SIZE, SUI_T_SPACE * 2, text, -1);

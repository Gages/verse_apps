...
extern void co_clone_node_update();
				...
				if(co_node != NULL && co_node->hidden != TRUE && co_node->search)
				{
					if(input->mode == BAM_DRAW)
					{
						...
						if(mode == COIM_CREATE)
						{
							glPushMatrix();
							glScalef(4, 4, 4);
							sui_draw_text(sui_text_line_length(SUI_T_SIZE, SUI_T_SPACE, e_ns_get_node_name(node), -1) * -0.5, 0, SUI_T_SIZE, SUI_T_SPACE, e_ns_get_node_name(node), co_line_color[0], co_line_color[1], co_line_color[2], 1);
					...
					if(input->mode == BAM_DRAW && mode == COIM_CREATE)
					{
						...
						if(y < 2 && y > -2)
						{
							...
							glScalef(4, 4, 4);
							{
								...
								sui_draw_text(sui_text_line_length(SUI_T_SIZE, SUI_T_SPACE, text, -1) * -0.5, -0.04, SUI_T_SIZE, SUI_T_SPACE, text, co_line_color[0], co_line_color[1], co_line_color[2], 1);
							}
							sui_draw_text(sui_text_line_length(SUI_T_SIZE, SUI_T_SPACE, e_ns_get_node_name(node), -1) * -0.5, 0, SUI_T_SIZE, SUI_T_SPACE, e_ns_get_node_name(node), co_line_color[0], co_line_color[1], co_line_color[2], 0.5);
							glPopMatrix();
						}
					}
				}
	...
	if(mode == COIM_CREATE)
	{
		...
		for(type = 0; type < V_NT_NUM_TYPES; type++)
		{
			...
			if(input->mode == BAM_DRAW)
			{
				...
				sui_draw_text(sui_text_line_length(SUI_T_SIZE, SUI_T_SPACE, "NEW", -1) * -0.5, 0, SUI_T_SIZE, SUI_T_SPACE, "NEW", co_line_color[0], co_line_color[1], co_line_color[2], 1);
				sui_draw_text(sui_text_line_length(SUI_T_SIZE, SUI_T_SPACE, names[type], -1) * -0.5, 0.15, SUI_T_SIZE, SUI_T_SPACE, names[type], co_line_color[0], co_line_color[1], co_line_color[2], 1);
	...
	if(mode == COIM_CREATE)
	{
		...
		if(input->mode == BAM_DRAW)
		{
			if(search[0] != 0)
			{
				...
				glPushMatrix();
				glTranslatef(0, 0, -1);
				length = sui_text_line_length(SUI_T_SIZE * 2, SUI_T_SPACE, search, -1);

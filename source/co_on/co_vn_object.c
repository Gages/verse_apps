...
boolean co_handle_object(BInputState *input, ENode *node)
{
	...
	if(rot_meth > 0.001)
	{
		...
		for(m_group = e_nso_get_next_method_group(node, 0); m_group != (uint16)-1 ; m_group = e_nso_get_next_method_group(node, m_group + 1))
		{
			...
			co_w_type_in(input, 0.2, y_group, 0.45, SUI_T_SIZE, e_nso_get_method_group(node, m_group), 16, rename_method_group_func, e_nso_get_method_group(node, m_group), color, color_light);
			...
			for(method = e_nso_get_next_method(node, m_group, 0); method != (uint16)-1; method = e_nso_get_next_method(node, m_group, method + 1))
			{
				...
				co_w_type_in(input, 0.2, y_meth, 0.45, SUI_T_SIZE, e_nso_get_method(node, m_group, method), 16, rename_method_func, e_nso_get_method(node, m_group, method), color, color_light);
